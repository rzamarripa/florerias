...
export async function generateDirectResponse(message, conversationHistory = []) {
  ...
  while (stepCount < MAX_QUERY_STEPS) {
    ...
    if (allResults.length > 0) {
      ...
      if (lastResult && lastResult.results) {
        contextualMessages.push({
          role: "assistant",
          content: `Previous step found: ${JSON.stringify(lastResult.results, null, 2)}`
        });
      }
    }
    ...
    if (Array.isArray(queryResults) && queryResults.length === 0 && stepCount > 1) {
      break;
    }
    
    currentQuery = queryData.missingInformation || `Get names for the IDs found in previous step`;
  }
...
export async function generateResponseFromResults(originalMessage, queryExecution, conversationHistory = []) {
  const result = await streamText({
    model: openai('gpt-4o-mini'),
    messages: [
      { 
      ...
      { 
        role: "user", 
        content: `Un usuario del sistema CAPREPA preguntó: "${originalMessage}"\n\nDatos encontrados tras ${queryExecution.totalSteps} consulta(s): ${JSON.stringify(queryExecution.combinedResults, null, 2)}\n\nPor favor explica estos resultados como un consultor financiero en lenguaje de negocio claro y útil.` 
      }
    ],
    temperature: 0.3,
    maxTokens: 1500,
