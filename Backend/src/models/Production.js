...
productionSchema.methods.comparePassword = async function(candidatePassword) {
  return await bcrypt.compare(candidatePassword, this.contrasena);
};

// Método para obtener el nombre completo
productionSchema.methods.getFullName = function() {
  return `${this.nombre} ${this.apellidoPaterno} ${this.apellidoMaterno}`.trim();
};

// Virtual para el nombre completo
productionSchema.virtual('nombreCompleto').get(function() {
  return this.getFullName();
});

// Transformar la salida JSON para ocultar la contraseña
productionSchema.methods.toJSON = function() {
  const production = this.toObject();
  delete production.contrasena;
  return production;
};
