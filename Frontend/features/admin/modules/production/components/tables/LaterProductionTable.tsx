...
import { Package, Truck, Clock, Calendar, Loader2, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, MoreHorizontal } from "lucide-react";
...
const LaterProductionTable: React.FC<LaterProductionTableProps> = ({
  ...
  useOrderSocket({
    ...
    onOrderCreated: (newOrder: any) => {
      // Si la orden es posterior a manana y tiene anticipo o esta enviada a produccion
  ...
  const fetchLaterOrders = async () => {
    try {
      ...
      if (response.data) {
        // Las ordenes ya vienen filtradas por fecha de entrega >= PASADO MANANA
      ...
      setError(err.message || "Error al cargar las ordenes posteriores");
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-3 text-muted-foreground">Cargando ordenes posteriores...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="default" size="sm" onClick={fetchLaterOrders} className="mt-2">
          Reintentar
        </Button>
      </div>
    );
  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-12">
        <Calendar size={48} className="text-muted-foreground mb-3 mx-auto" />
        <h5 className="text-muted-foreground font-medium">
          No hay ordenes programadas para dias posteriores
        </h5>
        <p className="text-muted-foreground">
          Las ordenes con fecha de entrega posterior a manana apareceran aqui
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-3 flex justify-between items-center">
        <div>
          <h5 className="mb-0 font-medium">Produccion Posterior</h5>
          <small className="text-muted-foreground">
            {totalOrders} orden(es) posteriores
          </small>
        </div>
      </div>
