...
const CashPaymentsInPackageTable: React.FC<CashPaymentsInPackageTableProps> = ({ pagos, onAuthorize, onReject, loading, packageStatus }) => {
    // Función para calcular el total de importeAPagar
    const calcularTotalImporteAPagar = () => pagos.reduce((sum, p) => sum + getNumber(p.importeAPagar), 0);
    // Función para calcular el total de importePagado
    const calcularTotalImportePagado = () => pagos.reduce((sum, p) => sum + getNumber(p.importePagado), 0);
                ...
                <Card className="shadow-sm border-0">
                    <Card.Body className="text-center py-5">
                        <div className="text-muted">
                            ...
                            <p className="mt-3 mb-0">No hay pagos en efectivo en este paquete</p>
                        </div>
                    </Card.Body>
                </Card>
            ) : (
                <Card className="shadow-sm border-0">
                    <Card.Body className="p-0">
                        <Table responsive hover className="mb-0 align-middle">
                            ...
                            <tbody>
                                {pagos.map((pago, idx) => (
                                    <tr key={pago._id} className={idx % 2 === 1 ? 'bg-pink bg-opacity-25' : ''}>
                                        <td>{idx + 1}</td>
                                        <td>
                                            <div className="fw-bold">
                                                {pago.expenseConcept?.name || 'Concepto no especificado'}
                                            </div>
                                            <div>
                                                <Badge bg="secondary" className="me-1">Pago {idx + 1}</Badge>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span className="text-primary">
                                                {pago.autorizada === null ? 'Pendiente' : 
                                                 pago.autorizada === true ? 'Autorizado' : 
                                                 'Pago Rechazado'}
                                            </span>
                                        </td>
                                        <td className="text-end">
                                            ${getNumber(pago.importeAPagar).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                                        </td>
                                        <td className="text-end">
                                            ${(getNumber(pago.importeAPagar) - getNumber(pago.importePagado)).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                                        </td>
                                        <td className="text-end">
                                            ${getNumber(pago.importePagado).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                                        </td>
                                        <td className="text-center">
                                            <div className="d-flex justify-content-center align-items-center">
                                                <Button
                                                    ...
                                                    disabled={loading || pago.autorizada === true || packageStatus !== 'Borrador'}
                                                >
                                                ...
                                                <Button
                                                    ...
                                                    disabled={loading || pago.autorizada === false || packageStatus !== 'Borrador'}
                                                >
                                ...
                                <tr className="fw-bold text-left">
                                    <td className="text-end" colSpan={5}>Total a pagar</td>
                                    <td className="text-end">${calcularTotalImporteAPagar().toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                                    <td className="text-end">${calcularTotalSaldo().toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                                    <td className="text-end">${calcularTotalImportePagado().toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                                    <td></td>
                                </tr>
                            </tbody>
                        </Table>
