...
import {
  cashPaymentSchema,
  CashPaymentFormData,
} from "../schemas/cashPaymentSchema";
...
} from "../../InvoicesPackpages/services/budget";
  ...
  const onSubmit = async (data: CashPaymentFormData) => {
    ...
    try {
      onSuccess({
        ...data,
        expenseConcept:
          typeof data.expenseConcept === "string" ? data.expenseConcept : "",
      });
      reset();
      onHide();
    } catch (e: any) {
            ...
            <Col md={6}>
              <Form.Group>
                ...
                >
                  ...
                  {(Array.isArray(expenseConcepts) ? expenseConcepts : []).map(
                    (c: any) => (
                      <option key={c._id} value={c._id}>
                        {c.categoryId?.name ? `${c.categoryId.name} - ` : ""}
                        {c.name}
                      </option>
                    )
                  )}
            ...
            {budgetData && (
              <Col md={12}>
                <div className="border rounded p-3 bg-light mb-3">
                  ...
                  {budgetData &&
                    watchedValues.importeAPagar &&
                    Number(watchedValues.importeAPagar) > 0 && (
                        ...
                        <strong
                          className={
                            budgetData.availableBudget -
                              Number(watchedValues.importeAPagar) >
                            0
                              ? "text-success"
                              : "text-danger"
                          }
