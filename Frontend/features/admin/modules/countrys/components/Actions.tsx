...
const CountryActions: React.FC<CountryActionsProps> = ({
    ...
    const handleToggleCountry = async (id: string, currentStatus: boolean) => {
        try {
            ...
            const response = await countryService.toggleStatus(id, currentStatus);
            ...
            if (response.success) {
                ...
                const errorMessage = response.message || `Error al ${currentStatus ? 'desactivar' : 'activar'} el país`;
                toast.error(errorMessage);
            }
        } catch (error: any) {
            console.error("Error toggling country:", error);

            let errorMessage = `Error al ${currentStatus ? 'desactivar' : 'activar'} el país "${country.name}"`;
