...
import {
  ...
  municipalitySchema,
} from "../schemas/municipalitySchema";
import {
  createMunicipality,
  updateMunicipality,
...
const MunicipalityModal: React.FC<MunicipalityModalProps> = ({
  ...
  const onSubmit = async (data: MunicipalityFormData) => {
    try {
      ...
      if (response.success) {
        ...
        const errorMessage =
          response.message ||
          `Error al ${isEditing ? "actualizar" : "crear"} el municipio`;
      ...
      if (error.response?.status === 400) {
        errorMessage =
          ...
          "Ya existe este municipio en el sistema";
  ...
  const defaultButtonProps = getModalButtonStyles("Municipio");
