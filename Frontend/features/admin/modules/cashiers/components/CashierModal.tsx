...
const CashierModal: React.FC<CashierModalProps> = ({
  ...
  useEffect(() => {
    if (cashier) {
      setFormData({
        ...
        apellidoPaterno: cashier.apellidoPaterno,
        apellidoMaterno: cashier.apellidoMaterno,
        direccion: cashier.direccion,
        telefono: cashier.telefono,
        correo: cashier.correo,
        usuario: cashier.usuario,
        contrasena: "",
        foto: cashier.foto || "",
        cajero: cashier.cajero,
        estatus: cashier.estatus,
      });
    } else {
      setFormData({
        nombre: "",
  ...
  const handleChange = (field: keyof CreateCashierData, value: any) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!cashier && !formData.contrasena.trim()) {
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmit}>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              type="url"
              placeholder="Ingresa la URL de la foto (opcional)"
              value={formData.foto}
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                <Form.Check
                  type="checkbox"
                  label="¿Es Cajero?"
                  checked={formData.cajero}
                  onChange={(e) => handleChange("cajero", e.target.checked)}
                />
              </Form.Group>
            </Col>
