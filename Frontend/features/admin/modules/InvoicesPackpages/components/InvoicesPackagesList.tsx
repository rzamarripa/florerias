...
const InvoicesPackagesList: React.FC<InvoicesPackagesListProps> = ({
  show = true,
}) => {
  ...
  const [selectedPackpageId, setSelectedPackpageId] = useState<string | null>(
    null
  );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Borrador":
        return (
          <span className="badge bg-secondary bg-opacity-10 text-secondary fw-bold py-2 px-3">
            Borrador
          </span>
        );
      case "Enviado":
        return (
          <span className="badge bg-info bg-opacity-10 text-info fw-bold py-2 px-3">
            Enviado
          </span>
        );
      case "Aprobado":
      ...
      case "Pagado":
        return (
          <span className="badge bg-primary bg-opacity-10 text-primary fw-bold py-2 px-3">
            Pagada
          </span>
        );
      case "Cancelado":
      ...
      case "PorFondear":
        return (
          <span className="badge bg-dark bg-opacity-10 text-dark fw-bold py-2 px-3">
            Por Fondear
          </span>
        );
      case "Fondeado":
  ...
  return (
    <Container fluid className="mt-4">
        ...
        <Card.Body className="p-0">
          {loading ? (
            ...
            <div className="text-center py-5">
              ...
              <p className="mt-2 text-muted">
                No se encontraron paquetes de facturas
              </p>
              ...
              <tbody>
                {filteredPackages.map((pkg) => {
                  ...
                  return (
                    <tr key={pkg._id}>
                      <td>
                        <strong className="text-primary">#{pkg.folio}</strong>
                      </td>
                      <td>
                        <div>
                      ...
                      <td>
                        <div>
                          ...
                          {new Date(pkg.fechaPago) < new Date() &&
                          pkg.estatus !== "Pagado" ? (
      ...
      >
        ...
        <Modal.Body>
          ¿Estás seguro que deseas eliminar este paquete? Esta acción no se
          puede deshacer.
          <br />
          Si el paquete contiene facturas, sus campos de registro serán
          limpiados.
        </Modal.Body>
        <Modal.Footer>
          <Button
