...
const EnviarPagoModal: React.FC<EnviarPagoModalProps> = ({ show, onClose, facturas, paqueteExistente, razonSocialName, isNewPackage = true, onSuccess, selectedCompanyId, selectedBrandId, selectedBranchId, tempPayments, tempCashPayments, onRemoveTempPayment, onRemoveTempCashPayment, onCancel }) => {
    ...
    }, 0) + (tempCashPayments?.reduce((sum, payment) => sum + payment.importeAPagar, 0) || 0);
    ...
    // Función helper para convertir valores a números (igual que en el detalle del paquete)
    ...
    const calcularTotalesFacturas = () => {
        if (!facturasLocal || facturasLocal.length === 0) return { total: 0, pagado: 0, pendiente: 0, cantidad: 0 };

        const total = facturasLocal.reduce((sum, f) => sum + toNumber(f.importeAPagar), 0);
    ...
    const calcularTotalesPagosEfectivo = () => {
        if (!tempCashPayments || tempCashPayments.length === 0) return { total: 0, pagado: 0, pendiente: 0, cantidad: 0 };

        const total = tempCashPayments.reduce((sum, p) => sum + toNumber(p.importeAPagar), 0);
        // Para pagos temporales, el importe pagado es igual al importe a pagar (se pagan completos)
        const pagado = tempCashPayments.reduce((sum, p) => sum + toNumber(p.importeAPagar), 0);
        const pendiente = total - pagado;
                ...
                <>
                    ...
                    <ul className="nav nav-tabs mb-3">
                        <li className="nav-item">
                            <a href="#facturas" className={`nav-link${activeTab === 'facturas' ? ' active' : ''}`} onClick={() => setActiveTab('facturas')}>
                                Facturas ({calcularTotalesFacturas().cantidad})
                            </a>
                        </li>
                        <li className="nav-item">
                            <a href="#pagosEfectivo" className={`nav-link${activeTab === 'pagosEfectivo' ? ' active' : ''}`} onClick={() => setActiveTab('pagosEfectivo')}>
                                ...
                                <Table responsive className="mb-0">
                                    ...
                                    <tbody>
                                        {facturasLocal.map((f, idx) => {
                                            ...
                                            return (
                                                <tr key={f._id} className={idx % 2 === 1 ? 'bg-pink bg-opacity-25' : ''}>
                                                    <td>{idx + 1}</td>
                                                    {/* Estado de guardado */}
                                                    <td>
                                        ...
                                        <tr className="fw-bold text-left">
                                            <td className="text-end" colSpan={9}>Total a pagar</td>
                                            <td className="text-end">{calcularTotalesFacturas().pendiente.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}</td>
                                            <td className="text-end">{calcularTotalesFacturas().pagado.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}</td>
                                            <td></td>
                                        </tr>
                                    </tbody>
                                </Table>
