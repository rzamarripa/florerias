...
const ImportInvoicesPage: React.FC = () => {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      if (!selectedCompany) {
        toast.warn(
          "Por favor, seleccione una Razón Social antes de subir un archivo."
        );
        return;
      }
      ...
      new JSZip()
        .loadAsync(file)
        .then((zip: JSZip) => {
          const txtFile = Object.values(zip.files).find((f: any) =>
        ...
        .then((content: string) => {
          ...
          if (lines.length <= 1)
            throw new Error(
              "El archivo de texto está vacío o no tiene el formato esperado."
            );
          ...
          const data: RawInvoiceData[] = lines.map((line: string) => {
            ...
            header.forEach((key, index) => {
              const keyMap: { [key: string]: keyof RawInvoiceData } = {
                ...
                FechaCertificacionSat: "FechaCertificacionSat",
                Monto: "Monto",
          ...
          const allInvoicesMatch = data.every(
            (invoice) =>
              invoice.RfcReceptor.trim().toUpperCase() === selectedRfc
          );

          if (!allInvoicesMatch) {
            throw new Error(
              `No todas las facturas en el archivo pertenecen a la Razón Social seleccionada (${selectedRfc}).`
            );
          }
          ...
          showPreview(data, 1);
          toast.success(
            `${data.length} facturas cargadas en vista previa. Haga clic en "Guardar" para importarlas.`
          );
        })
        .catch((err: any) => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await importedInvoicesService.bulkUpsert(
        parsedData,
        selectedCompany
      );
      if (response.success) {
        ...
        toast.error(
          response.message || "Ocurrió un error al guardar las facturas."
        );
      }
    } catch (error: any) {
            ...
            <Col md={8}>
              ...
              <div
                {...getRootProps({
                  ...
                  } ${!selectedCompany ? "bg-light" : "cursor-pointer"}`,
                })}
              >
                <input {...getInputProps()} />
                ...
                {isUploading ? (
                  ...
                  <p className="mt-2 mb-0">
                    {!selectedCompany
                      ? "Seleccione una Razón Social para continuar"
