...
import { StateFormData, stateSchema } from "../schemas/stateSchema";
import { createState, getAllCountries, updateState } from "../services/states";
...
const StateModal: React.FC<StateModalProps> = ({
  ...
  const onSubmit = async (data: StateFormData) => {
    try {
      ...
      if (error.response?.status === 400) {
        errorMessage =
          error.response.data?.message || "Ya existe este estado en el sistema";
  ...
  const defaultButtonProps = getModalButtonStyles("Estado");
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Form.Select {...field} isInvalid={!!errors.countryId}>
